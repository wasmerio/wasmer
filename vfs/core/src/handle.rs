...
use crate::node::{FsHandle, FsHandleAsync};
...
use vfs_ratelimit::rt::{NoopAsyncWait, StdSyncWait};
...
impl AsyncOFDState {
    pub fn new(backend: Arc<dyn FsHandleAsync>, flags: OpenFlags) -> Self {
        let status = flags.status_flags();
        Self {
            backend,
            offset: AtomicU64::new(0),
...
#[derive(Clone)]
pub struct VfsHandleAsync {
    id: VfsHandleId,
    _mount_guard: MountGuard,
    inode: VfsInodeId,
    file_type: VfsFileType,
    access: HandleAccess,
    state: Arc<AsyncOFDState>,
    rate_limiters: LimiterChain,
}
...
impl VfsHandleAsync {
    pub fn new(
        ...
        Self {
            ...
            file_type,
            access,
            state: Arc::new(AsyncOFDState::new(inner, flags)),
            rate_limiters,
        }
    }
    ...
    pub async fn get_metadata(&self) -> VfsResult<crate::VfsMetadata> {
        self.state.backend.get_metadata().await
    }

    pub async fn set_len(&self, new_len: u64) -> VfsResult<()> {
        self.require_access(HandleAccess::WRITE, "handle_async.set_len")?;
        if self.file_type == VfsFileType::Directory {
            return Err(VfsError::new(VfsErrorKind::IsDir, "handle_async.set_len"));
        }
        self.state.backend.set_len(new_len).await
    }

    pub async fn read(&self, buf: &mut [u8]) -> VfsResult<usize> {
        self.require_access(HandleAccess::READ, "handle_async.read")?;
        if self.file_type == VfsFileType::Directory {
            return Err(VfsError::new(VfsErrorKind::IsDir, "handle_async.read"));
        }
        self.apply_rate_limit_async(IoClass::Read, buf.len() as u64)
            .await?;

        let _guard = self.state.io_lock.lock().await;
        let offset = self.state.offset.load(Ordering::Acquire);
        let read = self.state.backend.read_at(offset, buf).await?;
        self.state
            .offset
            .store(offset.saturating_add(read as u64), Ordering::Release);
        Ok(read)
    ...
    pub async fn write(&self, buf: &[u8]) -> VfsResult<usize> {
        self.require_access(HandleAccess::WRITE, "handle_async.write")?;
        if self.file_type == VfsFileType::Directory {
            return Err(VfsError::new(VfsErrorKind::IsDir, "handle_async.write"));
        }
        self.apply_rate_limit_async(IoClass::Write, buf.len() as u64)
            .await?;

        let _guard = self.state.io_lock.lock().await;
        if self.status_flags().contains(HandleStatusFlags::APPEND) {
            let end = self.state.backend.get_metadata().await?.size;
            let written = self.state.backend.write_at(end, buf).await?;
            self.state
                .offset
                .store(end.saturating_add(written as u64), Ordering::Release);
            return Ok(written);
        ...
        let offset = self.state.offset.load(Ordering::Acquire);
        let written = self.state.backend.write_at(offset, buf).await?;
        self.state
            .offset
            .store(offset.saturating_add(written as u64), Ordering::Release);
        Ok(written)
    ...
    pub async fn pread_at(&self, offset: u64, buf: &mut [u8]) -> VfsResult<usize> {
        self.require_access(HandleAccess::READ, "handle_async.pread")?;
        if self.file_type == VfsFileType::Directory {
            return Err(VfsError::new(VfsErrorKind::IsDir, "handle_async.pread"));
        }
        self.apply_rate_limit_async(IoClass::Read, buf.len() as u64)
            .await?;
        self.state.backend.read_at(offset, buf).await
    }

    pub async fn pwrite_at(&self, offset: u64, buf: &[u8]) -> VfsResult<usize> {
        self.require_access(HandleAccess::WRITE, "handle_async.pwrite")?;
        if self.file_type == VfsFileType::Directory {
            return Err(VfsError::new(VfsErrorKind::IsDir, "handle_async.pwrite"));
        }
        self.apply_rate_limit_async(IoClass::Write, buf.len() as u64)
            .await?;
        self.state.backend.write_at(offset, buf).await
    }

    pub async fn pread(&self, offset: u64, buf: &mut [u8]) -> VfsResult<usize> {
        self.pread_at(offset, buf).await
    }

    pub async fn pwrite(&self, offset: u64, buf: &[u8]) -> VfsResult<usize> {
    ...
    pub async fn seek(&self, pos: SeekFrom) -> VfsResult<u64> {
        ...
        let new = match pos {
            ...
            SeekFrom::End(delta) => {
                let len = self.state.backend.get_metadata().await?.size;
                if delta >= 0 {
                    len.saturating_add(delta as u64)
                } else {
                    let neg = (-delta) as u64;
    ...
    pub async fn flush(&self) -> VfsResult<()> {
        self.state.backend.flush().await
    }

    pub async fn fsync(&self) -> VfsResult<()> {
        self.state.backend.fsync().await
    }

    pub fn dup(&self) -> VfsResult<Arc<VfsHandleAsync>> {
        Ok(Arc::new(self.clone()))
    }
    ...
    async fn apply_rate_limit_async(&self, class: IoClass, bytes: u64) -> VfsResult<()> {
        if self.rate_limiters.is_empty() {
            return Ok(());
        }
        let opts = AcquireOptions {
        ...
        let wait = NoopAsyncWait;
        self.rate_limiters
            .acquire_async(cost, opts, &wait)
            .await
            .map_err(|err| VfsError::new(map_acquire_error(err), "handle_async.ratelimit"))
    }
}
...
#[derive(Clone)]
pub struct VfsDirHandleAsync {
    inner: Arc<DirHandleAsyncInner>,
}

struct DirHandleAsyncInner {
    id: VfsHandleId,
    _mount_guard: MountGuard,
    inode: VfsInodeId,
    node: Arc<dyn crate::node::FsNodeAsync>,
    parent: Option<crate::inode::NodeRefAsync>,
}

impl VfsDirHandleAsync {
    pub fn new(
        id: VfsHandleId,
        mount_guard: MountGuard,
        inode: VfsInodeId,
        ...
        Self {
            inner: Arc::new(DirHandleAsyncInner {
                id,
                _mount_guard: mount_guard,
                inode,
                node,
    ...
    pub fn node(&self) -> &Arc<dyn crate::node::FsNodeAsync> {
        &self.inner.node
    }

    pub fn parent(&self) -> Option<crate::inode::NodeRefAsync> {
        self.inner.parent.clone()
    }
}
