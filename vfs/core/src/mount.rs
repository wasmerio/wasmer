...
impl MountTable {
    ...
    pub fn snapshot(&self) -> Arc<MountTableInner> {
        self.inner
            ...
            .unwrap_or_else(|_| Arc::new(MountTableInner {
                root: MountId(0),
                mounts: Vec::new(),
                mount_by_mountpoint: HashMap::new(),
                children_by_parent: HashMap::new(),
    ...
    pub fn enter_if_mountpoint(
        ...
        inner.mount_by_mountpoint.get(&child_inode).copied()
    }

    pub fn mount_root(inner: &MountTableInner, mount: MountId) -> Option<(VfsInodeId, Arc<dyn Fs>)> {
        let entry = inner.mounts.get(mount.0 as usize)?.as_ref()?;
    ...
    pub fn mount(
        ...
        if entry_id.0 as usize == inner.mounts.len() {
            inner.mounts.push(Some(entry.clone()));
        } else {
            inner.mounts[entry_id.0 as usize] = Some(entry.clone());
        }
...
impl Drop for MountGuard {
    fn drop(&mut self) {
        self.entry.open_count.fetch_sub(1, Ordering::Release);
    }
}
