...
- `lib/wasix/src/syscalls/wasi/fd_sync.rs`
  - Replace inode-based sync with VFS handle sync/flush as needed.
  - Ensure `FD_SYNC`/`FD_DATASYNC`/`FD_FSYNC` rights checks map to VFS calls or no-ops.
- `lib/wasix/src/syscalls/wasi/fd_event.rs`
  - Rework event polling to use `FdTable` + `Kind` and `fs::poll` guard types.
  - Ensure stdio/event notification paths use new `Stdio`/`NotificationInner`.
- `lib/wasix/src/syscalls/wasi/fd_fdstat_set_flags.rs`
  - Update to mutate `FdTable` entry flags only (no inode usage).
  - Make sure `NONBLOCK`/`APPEND`/`SYNC`/`DSYNC` map to VFS handle flags if required.
- `lib/wasix/src/syscalls/wasi/path_filestat_set_times.rs`
  - Implement via VFS `set_metadata`/`set_times` once available; otherwise return `Errno::Notsup`.
  - Remove all inode traversal and `state.fs` host-path manipulation.
- `lib/wasix/src/syscalls/wasix/proc_spawn2.rs`
  - Update executable lookup and path resolution to VFS (`VfsBaseDirAsync` + `statat`/`openat`).
  - Remove `WasiInodes` use and any host-path assumptions.
- `lib/wasix/src/syscalls/wasi/fd_write.rs` (legacy branch)
  - The `#[cfg(feature = "legacy-fs")]` branch still uses inodes; decide if it should remain
    or be removed for Phase 5 completion.

### `virtual_fs` usage outside syscalls
- `lib/wasix/src/state/linker.rs`
  - Replace any `virtual_fs` types and adapters with VFS equivalents or remove if unused.
...
- `lib/wasix/src/runtime/package_loader/load_package_tree.rs`
  - Remove any remaining `virtual_fs` references in tests and update to VFS providers.
