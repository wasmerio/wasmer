...
Shared Library Engine generates a shared object library and links it via
...
The Shared Library Engine and JIT engine mainly differ on how the
Modules are loaded/stored. Using the same compilers, both will have
the same runtime speed.

However, the Shared Library engine uses the Operating System shared
...
The `wasmer-engine-shared-library` crate requires a linker available
on your system to generate the shared object file.
...
[example]: https://github.com/wasmerio/wasmer/blob/master/examples/engine_shared_library.rs
