...
//!
//! This engine compiles a WebAssembly module into a shared object library.

use crate::SharedLibraryArtifact;
...
/// A WebAssembly Shared Library Engine.
#[derive(Clone)]
pub struct SharedLibraryEngine {
    inner: Arc<Mutex<SharedLibraryEngineInner>>,
    /// The target for the compiler
    target: Arc<Target>,
    engine_id: EngineId,
}

impl SharedLibraryEngine {
    /// Create a new `SharedLibraryEngine` with the given config
    #[cfg(feature = "compiler")]
    pub fn new(compiler: Box<dyn Compiler>, target: Target, features: Features) -> Self {
        ...
        let linker = if is_cross_compiling {
            which(Into::<&'static str>::into(Linker::Clang10))
                ...
                .expect("Nor `clang-10` or `clang` has been found, at least one of them is required for the `SharedLibraryEngine`")
        } else {
            which(Into::<&'static str>::into(Linker::Gcc))
                .map(|_| Linker::Gcc)
                .expect("`gcc` has not been found, it is required for the `SharedLibraryEngine`")
        };

        Self {
            inner: Arc::new(Mutex::new(SharedLibraryEngineInner {
    ...
    /// Create a headless `SharedLibraryEngine`
    ///
    ...
    pub(crate) fn inner(&self) -> std::sync::MutexGuard<'_, SharedLibraryEngineInner> {
        self.inner.lock().unwrap()
    }

    pub(crate) fn inner_mut(&self) -> std::sync::MutexGuard<'_, SharedLibraryEngineInner> {
        self.inner.lock().unwrap()
    }
}

impl Engine for SharedLibraryEngine {
    /// The target
    ...
    fn compile(
        ...
        Ok(Arc::new(SharedLibraryArtifact::new(
    ...
    fn compile(
        ...
        Err(CompileError::Codegen(
            "The `SharedLibraryEngine` is operating in headless mode, so it cannot compile a module."
                .to_string(),
        ))
    }
    ...
    unsafe fn deserialize(&self, bytes: &[u8]) -> Result<Arc<dyn Artifact>, DeserializeError> {
        Ok(Arc::new(SharedLibraryArtifact::deserialize(&self, &bytes)?))
    }
    ...
    unsafe fn deserialize_from_file(
        ...
        Ok(Arc::new(SharedLibraryArtifact::deserialize_from_file(
            &self, &file_ref,
        )?))
    }
...
/// The inner contents of `SharedLibraryEngine`
pub struct SharedLibraryEngineInner {
    /// The compiler
    #[cfg(feature = "compiler")]
    ...
    /// the functions in the shared object generated by the `SharedLibraryEngine`,
    /// so we can assure no collisions.
    prefixer: Option<Box<dyn Fn(&[u8]) -> String + Send>>,
    /// Whether the shared library engine will cross-compile.
...
impl SharedLibraryEngineInner {
    /// Gets the compiler associated to this engine.
    #[cfg(feature = "compiler")]
    pub fn compiler(&self) -> Result<&dyn Compiler, CompileError> {
        if self.compiler.is_none() {
            return Err(CompileError::Codegen("The `SharedLibraryEngine` is operating in headless mode, so it can only execute already compiled Modules.".to_string()));
        }
        Ok(&**self
            .compiler
            .as_ref()
    ...
    pub fn validate<'data>(&self, _data: &'data [u8]) -> Result<(), CompileError> {
        Err(CompileError::Validate(
            "The `SharedLibraryEngine` is not compiled with compiler support, which is required for validating".to_string(),
        ))
    }
