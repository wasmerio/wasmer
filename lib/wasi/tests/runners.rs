...
mod wasi {
    use wasmer::Store;
    use wasmer_wasi::{runners::wasi::WasiRunner, WasiError};

    use super::*;

    #[tokio::test]
    async fn can_run_wat2wasm() {
        let (_, webc) = download_cached("https://wapm.io/wasmer/wabt").await;
        let store = Store::default();
        let container = WapmContainer::new(webc).unwrap();
    ...
    async fn wat2wasm() {
        ...
        let handle =
            std::thread::spawn(move || WasiRunner::new(store).run_cmd(&container, "wat2wasm"));
        let err = handle.join().unwrap().unwrap_err();

        let runtime_error = cause_chain(&*err)
    ...
    #[tokio::test]
    async fn python() {
        let (_, webc) = download_cached("https://wapm.io/python/python").await;
        let store = Store::default();
        let container = WapmContainer::new(webc).unwrap();
        ...
        let handle = std::thread::spawn(move || {
            ...
            runner.run_cmd(&container, "python")
        });
        let err = handle.join().unwrap().unwrap_err();
    ...
    fn cause_chain<'a>(
        ...
        let mut next_error = Some(e);

        std::iter::from_fn(move || {
            let e = next_error.take()?;
            next_error = e.source();
            Some(e)
        })
    }
...
async fn download_cached(url: &str) -> (bytes::Bytes, PathBuf) {
    ...
    if cached_path.exists() {
        let bytes = std::fs::read(&cached_path).unwrap().into();
        return (bytes, cached_path);
    }
