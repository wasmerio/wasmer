...
use wasmer::{
    ...
    WasmPtr, WasmSlice, FunctionEnv, Instance, Module, Extern,
};
...
pub fn thread_local_create<M: MemorySize>(
    ...
    let memory = env.memory();
    wasi_try_mem!(ret_key.write(&ctx, memory, key));
    __WASI_ESUCCESS
}
...
pub fn thread_local_get<M: MemorySize>(
    ...
    let memory = env.memory();
    wasi_try_mem!(ret_val.write(&ctx, memory, val));
    __WASI_ESUCCESS
}
...
pub fn futex_wake<M: MemorySize>(
    ...
    wasi_try_mem!(ret_woken.write(&ctx, memory, woken));
...
pub fn bus_open_local<M: MemorySize>(
    ...
    let bus = env.runtime.bus();
    let memory = env.memory();
    let name = unsafe { get_input_str_bus_ok!(&ctx, memory, name, name_len) };
...
pub fn bus_open_remote<M: MemorySize>(
    ...
    let memory = env.memory();
    let name = unsafe { get_input_str_bus_ok!(&ctx, memory, name, name_len) };
    let instance = unsafe { get_input_str_bus_ok!(&ctx, memory, instance, instance_len) };
    let token = unsafe { get_input_str_bus_ok!(&ctx, memory, token, token_len) };
    let reuse = reuse == __WASI_BOOL_TRUE;
    debug!(
...
fn bus_open_internal<M: MemorySize>(
    ...
    if reuse {
        ...
        if let Some(bid) = guard.process_reuse.get(&name) {
            if guard.processes.contains_key(bid) {
                wasi_try_mem_bus_ok!(ret_bid.write(&ctx, memory, bid.clone().into()));
    ...
    wasi_try_mem_bus_ok!(ret_bid.write(&ctx, memory, bid.into()));
...
pub fn bus_call<M: MemorySize>(
    ...
    let bus = env.runtime.bus();
    let memory = env.memory();
    let topic_hash = wasi_try_mem_bus_ok!(topic_hash.read(&ctx, memory));
    let buf_slice = wasi_try_mem_bus_ok!(buf.slice(&ctx, memory, buf_len));
    trace!(
    ...
    wasi_try_mem_bus_ok!(ret_cid.write(&ctx, memory, cid));
    Ok(__BUS_ESUCCESS)
}
...
pub fn bus_poll<M: MemorySize>(
    ...
    let events = wasi_try_mem_bus_ok!(events.slice(&ctx, memory, maxevents));
    ...
    wasi_try_mem_bus_ok!(ret_nevents.write(&ctx, memory, nevents));
    Ok(__BUS_ESUCCESS)
}
...
pub fn call_reply<M: MemorySize>(
    ...
    let buf_slice = wasi_try_mem_bus!(buf.slice(&ctx, memory, buf_len));
