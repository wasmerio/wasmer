    ...
    Fut: std::future::Future<Output = Result<T, Errno>> + 'static,
{
    ...
    // Enter a loop that can be receive intr commands
    loop
    {
        ...
        return match ret {
            Some(a) => a,
            None => {
                if ctx.data().clone().process_signals(&mut ctx)? == true {
                    env = ctx.data();
                    continue;
                } else {
                    Err(Errno::Intr)
            ...
            match builder.spawn(Some(&ctx), name.as_str(), new_store, &bin_factory) {
                    ...
                    loop {
                        ...
                        if let Some(exit_code) = process.inst.exit_code() {
                            return OnCalledAction::Trap(Box::new(WasiError::Exit(
                                exit_code as crate::syscalls::types::ExitCode,
                            )));
                        }
                    }
                }
                Err(err) => {
                    ...
                    OnCalledAction::Trap(Box::new(WasiError::Exit(
                        Errno::Noexec as crate::syscalls::types::ExitCode,
                    )))
                }
            }
        })?;
        ...
        let mut conv_stdio_mode = |mode: __wasi_stdiomode_t,
         ...
         -> Result<__wasi_option_fd_t, __bus_errno_t> {
            match mode {
                ...
                __WASI_STDIO_MODE_INHERIT => Ok(OptionFd {
                    tag: OptionTag::None,
                    fd: u32::MAX,
                }),
...
pub fn proc_join<M: MemorySize>(
    ...
    if pid == u32::MAX {
        let process = ctx.data_mut().process.clone();
        let child_exit = wasi_try_ok!(__asyncify(
            &mut ctx,
            None,
            async move {
    ...
    if let Some(process) = process {
        ...
        if let Some(exit_code) = child_exit {
...
pub fn ws_connect<M: MemorySize>(
    ...
    let net = env.net();
    let tasks = env.tasks.clone();
    let socket = wasi_try!(__asyncify(
        &mut ctx,
        None,
        async move {
...
pub fn resolve<M: MemorySize>(
    ...
    let net = env.net();
    let tasks = env.tasks.clone();
    let found_ips = wasi_try!(__asyncify(
        &mut ctx,
        None,
        async move {
