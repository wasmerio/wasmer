...
pub mod types {
    ...
    pub use wasmer_wasi_types_generated::wasi_snapshot0;
}
...
fn to_offset<M: MemorySize>(offset: usize) -> Result<M::Offset, wasi_snapshot0::Errno> {
    let ret: M::Offset = offset
        .try_into()
        .map_err(|_| wasi_snapshot0::Errno::Inval)?;
    Ok(ret)
}

fn from_offset<M: MemorySize>(offset: M::Offset) -> Result<usize, wasi_snapshot0::Errno> {
    let ret: usize = offset
        .try_into()
        .map_err(|_| wasi_snapshot0::Errno::Inval)?;
    Ok(ret)
}
...
fn __sock_actor<T, F>(
    ...
    sock: wasi_snapshot0::Fd,
    rights: wasi_snapshot0::Rights,
    actor: F,
) -> Result<T, wasi_snapshot0::Errno>
where
    F: FnOnce(&crate::state::InodeSocket) -> Result<T, wasi_snapshot0::Errno>,
{
    let env = ctx.data();
    ...
    let ret = {
        ...
        match deref {
            ...
            _ => {
                return Err(wasi_snapshot0::Errno::Notsock);
            }
        }
    };
...
where
    F: FnOnce(&mut crate::state::InodeSocket) -> Result<T, wasi_snapshot0::Errno>,
{
    let env = ctx.data();
    let (_, state, inodes) = env.get_memory_and_wasi_state_and_inodes(&ctx, 0);
...
where
    ...
    ) -> Result<Option<crate::state::InodeSocket>, wasi_snapshot0::Errno>,
{
    let env = ctx.data();
    let (_, state, inodes) = env.get_memory_and_wasi_state_and_inodes(&ctx, 0);
...
fn get_current_time_in_nanos() -> Result<__wasi_timestamp_t, wasi_snapshot0::Errno> {
    let now = std::time::SystemTime::now();
    let duration = now
        .duration_since(std::time::SystemTime::UNIX_EPOCH)
        .map_err(|_| wasi_snapshot0::Errno::Io)?;
...
pub fn args_sizes_get<M: MemorySize>(
    ...
    let argc_val: M::Offset = wasi_try!(state
        ...
        .len()
        .try_into()
        .map_err(|_| wasi_snapshot0::Errno::Overflow));
    let argv_buf_size_val: usize = state.args.iter().map(|v| v.len() + 1).sum();
    let argv_buf_size_val: M::Offset = wasi_try!(argv_buf_size_val
        .try_into()
        .map_err(|_| wasi_snapshot0::Errno::Overflow));
...
pub fn clock_time_get<M: MemorySize>(
    ...
    let result = wasi_snapshot0::Errno::Success;
    trace!(
        "time: {} => {}",
...
pub fn environ_sizes_get<M: MemorySize>(
    ...
    let env_var_count: M::Offset = wasi_try!(state
        .envs
        .len()
        .try_into()
        .map_err(|_| wasi_snapshot0::Errno::Overflow));
    let env_buf_size: usize = state.envs.iter().map(|v| v.len() + 1).sum();
    let env_buf_size: M::Offset = wasi_try!(env_buf_size
        .try_into()
        .map_err(|_| wasi_snapshot0::Errno::Overflow));
        ...
        match deref_mut {
            Kind::File { handle, .. } => {
                if let Some(handle) = handle {
                    ...
                    return wasi_snapshot0::Errno::Badf;
                }
            }
            Kind::Socket { .. } => return wasi_snapshot0::Errno::Badf,
            Kind::Pipe { .. } => return wasi_snapshot0::Errno::Badf,
            Kind::Buffer { buffer } => {
                buffer.resize(new_size as usize, 0);
            }
            Kind::Symlink { .. } => return wasi_snapshot0::Errno::Badf,
            Kind::EventNotifications { .. } => return wasi_snapshot0::Errno::Badf,
            Kind::Dir { .. } | Kind::Root { .. } => return wasi_snapshot0::Errno::Isdir,
        }
    }
...
/// Errors:
/// - `wasi_snapshot0::Errno::Isdir`
///     If `fd` is a directory
/// - `wasi_snapshot0::Errno::Badf`
///     If `fd` is invalid or not open
pub fn fd_close(ctx: FunctionEnvMut<'_, WasiEnv>, fd: wasi_snapshot0::Fd) -> wasi_snapshot0::Errno {
...
pub fn fd_fdstat_set_rights(
    ...
    {
        return wasi_snapshot0::Errno::Notcapable;
    }
...
pub fn fd_filestat_set_times(
    ...
    {
        return wasi_snapshot0::Errno::Inval;
    }
...
pub fn fd_pread<M: MemorySize>(
    ...
    let bytes_read = match fd {
        __WASI_STDIN_FILENO => {
            ...
            if let Some(ref mut stdin) = guard.deref_mut() {
                ...
                return Ok(wasi_snapshot0::Errno::Badf);
            }
        }
        __WASI_STDOUT_FILENO => return Ok(wasi_snapshot0::Errno::Inval),
        __WASI_STDERR_FILENO => return Ok(wasi_snapshot0::Errno::Inval),
        _ => {
            let inode = fd_entry.inode;
            ...
            match deref_mut {
                Kind::File { handle, .. } => {
                    if let Some(h) = handle {
                        ...
                        return Ok(wasi_snapshot0::Errno::Inval);
                    }
                }
                Kind::Socket { socket } => {
                ...
                Kind::EventNotifications { .. } => return Ok(wasi_snapshot0::Errno::Inval),
                Kind::Dir { .. } | Kind::Root { .. } => return Ok(wasi_snapshot0::Errno::Isdir),
    ...
    Ok(wasi_snapshot0::Errno::Success)
}
...
pub fn fd_prestat_dir_name<M: MemorySize>(
    ...
    match deref {
        ...
        | Kind::Socket { .. }
        | Kind::Pipe { .. }
        | Kind::EventNotifications { .. } => wasi_snapshot0::Errno::Notdir,
    }
}
...
pub fn fd_pwrite<M: MemorySize>(
    ...
    let bytes_written = match fd {
        __WASI_STDIN_FILENO => return Ok(wasi_snapshot0::Errno::Inval),
        ...
        _ => {
            ...
            match deref_mut {
                ...
                Kind::Dir { .. } | Kind::Root { .. } => {
                    // TODO: verify
                    return Ok(wasi_snapshot0::Errno::Isdir);
                }
                Kind::EventNotifications { .. } => return Ok(wasi_snapshot0::Errno::Inval),
    ...
    let bytes_written: M::Offset = wasi_try_ok!(bytes_written
        .try_into()
        .map_err(|_| wasi_snapshot0::Errno::Overflow));
...
pub fn fd_read<M: MemorySize>(
    ...
    let bytes_read = match fd {
        ...
        _ => {
                ...
                match deref_mut {
                    ...
                    Kind::EventNotifications {
                        ...
                        loop {
                            ...
                            if is_non_blocking {
                                return Ok(wasi_snapshot0::Errno::Again);
                            }
            ...
            // reborrow
            let mut fd_map = state.fs.fd_map.write().unwrap();
            let fd_entry = wasi_try_ok!(fd_map.get_mut(&fd).ok_or(wasi_snapshot0::Errno::Badf));
...
pub fn fd_readdir<M: MemorySize>(
    ...
    let entries: Vec<(String, wasi_snapshot0::Filetype, u64)> = {
        ...
        match deref {
            ...
            | Kind::Socket { .. }
            | Kind::Pipe { .. }
            | Kind::EventNotifications { .. } => return wasi_snapshot0::Errno::Notdir,
        }
    };
    ...
    let buf_idx: M::Offset = wasi_try!(buf_idx
        .try_into()
        .map_err(|_| wasi_snapshot0::Errno::Overflow));
...
/// Inputs:
/// - `wasi_snapshot0::Fd from`
///     File descriptor to copy
/// - `wasi_snapshot0::Fd to`
///     Location to copy file descriptor to
pub fn fd_renumber(
    ctx: FunctionEnvMut<'_, WasiEnv>,
    from: wasi_snapshot0::Fd,
    to: wasi_snapshot0::Fd,
) -> wasi_snapshot0::Errno {
    ...
    let mut fd_map = state.fs.fd_map.write().unwrap();
    let fd_entry = wasi_try!(fd_map.get_mut(&from).ok_or(wasi_snapshot0::Errno::Badf));
...
pub fn fd_dup<M: MemorySize>(
    ctx: FunctionEnvMut<'_, WasiEnv>,
    fd: wasi_snapshot0::Fd,
    ret_fd: WasmPtr<wasi_snapshot0::Fd, M>,
) -> wasi_snapshot0::Errno {
    debug!("wasi::fd_dup");
...
pub fn fd_seek<M: MemorySize>(
    ...
    match whence {
            ...
            match deref_mut {
                Kind::File { ref mut handle, .. } => {
                    if let Some(handle) = handle {
                        ...
                        let mut fd_map = state.fs.fd_map.write().unwrap();
                        let fd_entry =
                            wasi_try_ok!(fd_map.get_mut(&fd).ok_or(wasi_snapshot0::Errno::Badf));
        ...
        _ => return Ok(wasi_snapshot0::Errno::Inval),
    }
    // reborrow
    let fd_entry = wasi_try_ok!(state.fs.get_fd(fd));
...
/// - `wasi_snapshot0::Errno::Perm`
/// - `wasi_snapshot0::Errno::Notcapable`
pub fn fd_sync(ctx: FunctionEnvMut<'_, WasiEnv>, fd: wasi_snapshot0::Fd) -> wasi_snapshot0::Errno {
    debug!("wasi::fd_sync");
    debug!("=> fd={}", fd);
    let env = ctx.data();
    let (_, mut state, inodes) = env.get_memory_and_wasi_state_and_inodes(&ctx, 0);
    ...
    // TODO: implement this for more than files
    {
        ...
        match deref_mut {
            ...
            Kind::Root { .. } | Kind::Dir { .. } => return wasi_snapshot0::Errno::Isdir,
            Kind::Buffer { .. }
            | Kind::Symlink { .. }
            | Kind::Socket { .. }
            | Kind::Pipe { .. }
            | Kind::EventNotifications { .. } => return wasi_snapshot0::Errno::Inval,
        }
    }
...
pub fn fd_pipe<M: MemorySize>(
    ctx: FunctionEnvMut<'_, WasiEnv>,
    ro_fd1: WasmPtr<wasi_snapshot0::Fd, M>,
    ro_fd2: WasmPtr<wasi_snapshot0::Fd, M>,
) -> wasi_snapshot0::Errno {
    trace!("wasi::fd_pipe");
...
pub fn path_create_directory<M: MemorySize>(
    ...
    let path_vec = wasi_try!(path
        ...
        .collect::<Result<Vec<String>, wasi_snapshot0::Errno>>());
    ...
    for comp in &path_vec {
        ...
        match deref_mut {
            ...
            Kind::Root { .. } => return wasi_snapshot0::Errno::Acces,
            _ => return wasi_snapshot0::Errno::Notdir,
        }
    }
...
pub fn path_link<M: MemorySize>(
    ctx: FunctionEnvMut<'_, WasiEnv>,
    old_fd: wasi_snapshot0::Fd,
    old_flags: __wasi_lookupflags_t,
    old_path: WasmPtr<u8, M>,
    old_path_len: M::Offset,
    new_fd: wasi_snapshot0::Fd,
    ...
    {
        return wasi_snapshot0::Errno::Mlink;
    }
    {
        ...
        match deref_mut {
            ...
            Kind::Root { .. } => return wasi_snapshot0::Errno::Inval,
            Kind::File { .. }
            | Kind::Symlink { .. }
            | Kind::Buffer { .. }
            | Kind::Socket { .. }
...
pub fn path_open<M: MemorySize>(
    ...
    fd: WasmPtr<wasi_snapshot0::Fd, M>,
) -> wasi_snapshot0::Errno {
    ...
    if path_len64 > 1024u64 * 1024u64 {
        return wasi_snapshot0::Errno::Nametoolong;
    }
        ...
        match deref_mut {
            Kind::File {
                ...
                if let Some(special_fd) = fd {
                    ...
                    return wasi_snapshot0::Errno::Success;
                }
...

    {
        ...
        if let Kind::Symlink { relative_path, .. } = guard.deref() {
            ...
            if bytes.len() as u64 >= buf_len {
                return wasi_snapshot0::Errno::Overflow;
            }
            ...
            let bytes_len: M::Offset = wasi_try!(bytes
                .len()
                .try_into()
                .map_err(|_| wasi_snapshot0::Errno::Overflow));
...
/// Returns wasi_snapshot0::Errno::Notemtpy if directory is not empty
        ...
        match deref {
            Kind::Dir { entries, path, .. } => {
                if !entries.is_empty() || wasi_try!(state.fs_read_dir(path)).count() != 0 {
                    return wasi_snapshot0::Errno::Notempty;
                }
                path.clone()
            }
            Kind::Root { .. } => return wasi_snapshot0::Errno::Acces,
...

    {
        ...
        match deref_mut {
            Kind::Dir {
                ...
                let removed_inode = wasi_try!(entries
                    .remove(&childs_name)
                    .ok_or(wasi_snapshot0::Errno::Inval));
...
pub fn path_rename<M: MemorySize>(
    ...
    let host_adjusted_target_path = {
        ...
        match deref {
            ...
            Kind::Root { .. } => return wasi_snapshot0::Errno::Notcapable,
...
pub fn poll_oneoff<M: MemorySize>(
    ...
    let fds = {
        ...
        for fd in fd_guards.iter() {
            f.push(wasi_try_ok!(fd.as_ref().ok_or(wasi_snapshot0::Errno::Badf)).deref());
        }
        f
    };
    ...
    let events_seen: M::Offset = wasi_try_ok!(events_seen
        .try_into()
        .map_err(|_| wasi_snapshot0::Errno::Overflow));
...
pub fn proc_raise(ctx: FunctionEnvMut<'_, WasiEnv>, sig: __wasi_signal_t) -> wasi_snapshot0::Errno {
...
pub fn sched_yield(ctx: FunctionEnvMut<'_, WasiEnv>) -> Result<wasi_snapshot0::Errno, WasiError> {
...
pub fn thread_spawn<M: MemorySize>(
    ...
    let funct = unsafe {
        if env.thread_start_ref().is_none() {
            return wasi_snapshot0::Errno::Addrnotavail;
        }
    ...
    */
    ...
    let child = {
        ...
        wasi_try!(env
            ...
            .map_err(|err| {
                let err: wasi_snapshot0::Errno = err.into();
                err
            }));
        id
    };
...
pub fn thread_parallelism<M: MemorySize>(
    ...
    let parallelism: M::Offset = wasi_try!(parallelism
        .try_into()
        .map_err(|_| wasi_snapshot0::Errno::Overflow));
    let memory = env.memory_view(&ctx);
...
pub fn ws_connect<M: MemorySize>(
    ...
    url: WasmPtr<u8, M>,
    url_len: M::Offset,
    ret_sock: WasmPtr<wasi_snapshot0::Fd, M>,
) -> wasi_snapshot0::Errno {
...
pub fn port_unbridge(ctx: FunctionEnvMut<'_, WasiEnv>) -> wasi_snapshot0::Errno {
...
pub fn port_dhcp_acquire(ctx: FunctionEnvMut<'_, WasiEnv>) -> wasi_snapshot0::Errno {
...
pub fn port_addr_clear(ctx: FunctionEnvMut<'_, WasiEnv>) -> wasi_snapshot0::Errno {
...
pub fn port_addr_list<M: MemorySize>(
    ...
    let max_addrs: u64 = wasi_try!(max_addrs
        .try_into()
        .map_err(|_| wasi_snapshot0::Errno::Overflow));
    let ref_addrs =
    ...
    let addrs_len: M::Offset = wasi_try!(addrs
        .len()
        .try_into()
        .map_err(|_| wasi_snapshot0::Errno::Overflow));
...
pub fn port_route_clear(ctx: FunctionEnvMut<'_, WasiEnv>) -> wasi_snapshot0::Errno {
...
pub fn port_route_list<M: MemorySize>(
    ...
    let routes_len: M::Offset = wasi_try!(routes
        .len()
        .try_into()
        .map_err(|_| wasi_snapshot0::Errno::Inval));
...
pub fn sock_open<M: MemorySize>(
    ...
    ro_sock: WasmPtr<wasi_snapshot0::Fd, M>,
) -> wasi_snapshot0::Errno {
    ...
    let kind = match ty {
        ...
        _ => return wasi_snapshot0::Errno::Notsup,
    };
...
pub fn sock_listen<M: MemorySize>(
    ...
    let env = ctx.data();
    let backlog: usize = wasi_try!(backlog.try_into().map_err(|_| wasi_snapshot0::Errno::Inval));
    wasi_try!(__sock_upgrade(
        &ctx,
        sock,
        wasi_snapshot0::Rights::SOCK_BIND,
...
pub fn sock_accept<M: MemorySize>(
    ...
    ro_fd: WasmPtr<wasi_snapshot0::Fd, M>,
    ro_addr: WasmPtr<__wasi_addr_port_t, M>,
) -> Result<wasi_snapshot0::Errno, WasiError> {
...
pub unsafe fn sock_send_file<M: MemorySize>(
    ...
    {
        let mut fd_map = state.fs.fd_map.write().unwrap();
        let fd_entry = wasi_try_ok!(fd_map.get_mut(&in_fd).ok_or(wasi_snapshot0::Errno::Badf));
        fd_entry.offset = offset as u64;
    }
    ...
    while (count > 0) {
        ...
        let bytes_read = match in_fd {
            ...
            _ => {
                ...
                let mut fd_map = state.fs.fd_map.write().unwrap();
                let fd_entry =
                    wasi_try_ok!(fd_map.get_mut(&in_fd).ok_or(wasi_snapshot0::Errno::Badf));
                fd_entry.offset += bytes_read as u64;
...
pub fn resolve<M: MemorySize>(
    ...
    let naddrs: usize = wasi_try!(naddrs.try_into().map_err(|_| wasi_snapshot0::Errno::Inval));
    let env = ctx.data();
    let memory = env.memory_view(&ctx);
    ...
    let idx: M::Offset = wasi_try!(idx.try_into().map_err(|_| wasi_snapshot0::Errno::Overflow));
