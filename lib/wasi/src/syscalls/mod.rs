...
#[inline]
pub fn tick(thread: &WasiThread) {
    let code = thread.terminate.load(Ordering::Relaxed);
    if code != TERMINATE_NOOP {
        proc_exit(thread, code);
    }
    if let Some(callback) = thread.on_tick.as_ref() {
        callback(thread);
    }
}
