...
pub struct Aarch64Machine {
    ...
    saved_free_regs: Vec<Vec<u32>>,
    saved_free_callee_save: Vec<Vec<u32>>,
...
impl Machine for Aarch64Machine {
    ...
    fn do_add_p(&mut self, src1: Local<Location>, src2: Local<Location>) -> Local<Location> {
        let (src1, src2, dst) = self.prep_bin_op(src1, src2, true, 12);
        // println!("{:?} {:?} {:?}", src1.location(), src2.location(), dst.location());
        ...
        match src2.location() {
            Location::Reg(Reg::X(src2_reg)) => {
                dynasm!(self.assembler ; .arch aarch64 ; add X(dst_reg), X(src1_reg), X(src2_reg));
            },
            Location::Imm32(src2_val) => {
                dynasm!(self.assembler ; .arch aarch64 ; add X(dst_reg), X(src1_reg), src2_val as u32);
            },
            _ => {  
                unreachable!()
            },
    ...
    fn do_deref_write(&mut self, sz: Size, ptr: Local<Location>, val: Local<Location>) {
        ...
        dynasm!(self.assembler ; .arch aarch64 ; str W(val_reg), [X(ptr_reg)]);
    }
