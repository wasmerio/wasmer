...
wasm_trap_t* host_func_callback(const wasm_val_vec_t* args,
...
int main(int argc, const char* argv[]) {
  const char* wat_string =
      ...
      "  (func $function (export \"guest_function\") (result i32) (global.get "
      "$global))\n"
  ...
  wasm_functype_t* host_func_type =
      wasm_functype_new_0_1(wasm_valtype_new_i32());
  wasm_func_t* host_func =
      wasm_func_new(store, host_func_type, host_func_callback);
  ...
  wasm_global_t* host_global =
      wasm_global_new(store, host_global_type, &host_global_val);
