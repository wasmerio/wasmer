...
int main(int argc, const char* argv[]) {
  ...
  // We can't validate the exact error message because it's not universal for
  // the engines assert(0 == strcmp("The maximum requested memory (2147483647
  // pages) is greater than the maximum allowed memory (65536 pages)", error));
  free(error);
  ...
  printf("Found error string: %s\n", error);
  // We can't validate the exact error message because it's not universal for
  // the engines assert(0 == strcmp("The minimum requested (2147483647 pages)
  // memory is greater than the maximum allowed memory (65536 pages)", error));
  free(error);
  ...
  printf("Found error string: %s\n", error);
  // We can't validate the exact error message because it's not universal for
  // the engines assert(0 == strcmp("The memory is invalid because the maximum
  // (10 pages) is less than the minimum (15 pages)", error));
  free(error);
