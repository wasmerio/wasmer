...
//         |mut env, values| {
//             ENV.with(|data| {
//                 let ((), mut store) = env.data_and_store_mut();
...
//                 let store_raw = store.as_store_mut().as_raw();
//                 let func = data.borrow().future_func.as_ref().unwrap().clone();
//                 let mut future = Box::pin(async move {
//                     let mut store = unsafe { StoreMut::<'static>::from_raw(store_raw) };
//                     let result = func.call_async(&mut store, &[Value::I32(future_id)]).await;
//                     result
//                 });
//                 log(future_id + 20);
...
//         .build()
//         .unwrap()
//         .block_on(main.call_async(&mut store, &[]))?;
