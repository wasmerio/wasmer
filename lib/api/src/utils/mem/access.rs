...
use crate::{utils::mem::WasmRef, MemoryAccessError, WasmSlice};
    ...
    T: wasmer_types::ValueType,
{
    fn drop(&mut self) {
        ...
        if let SliceCow::Owned(buf, modified) = &self.buf {
            if *modified {
                slice.write_slice(buf.as_ref()).ok();
            }
        }
    }
}
...
where
    T: wasmer_types::ValueType,
{
    pub(crate) fn new(slice: WasmSlice<T>) -> Result<Self, MemoryAccessError> {
        let total_len = slice
            .len
