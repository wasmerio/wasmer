...
use crate::{
    ...
    sys::vm::{VMContinuation, VMContinuationRef},
};

#[derive(Debug, Clone, PartialEq, Eq)]
/// A WebAssembly `exnref` in the `sys` runtime.
pub(crate) struct Continuation {
    exnref: wasmer_vm::VMContinuationRef,
}

impl Continuation {
    /// Creates a new continuation with the given tag and payload, and also creates
    ...
    pub fn new(store: &mut impl AsStoreMut, tag: &crate::sys::tag::Tag, payload: &[Value]) -> Self {
        if !tag.is_from_store(store) {
            panic!("cannot create Continuation with Tag from another Store");
        }
        ...
        let continuation = wasmer_vm::VMContinuation::new(tag.handle.internal_handle(), values);
        let exn_handle = wasmer_vm::StoreHandle::new(ctx, continuation);
        let exnref = wasmer_vm::VMContinuationRef(exn_handle);

        Self { exnref }
    }

    pub fn exnref(&self) -> wasmer_vm::VMContinuationRef {
        self.exnref.clone()
    }

    pub fn from_exnref(exnref: wasmer_vm::VMContinuationRef) -> Self {
        Self { exnref }
    }
    ...
    pub fn tag(&self, store: &impl AsStoreRef) -> crate::sys::tag::Tag {
        if !self.is_from_store(store) {
            panic!("Continuation is from another Store");
        }
        let ctx = store.as_store_ref().objects().as_sys();
        let continuation = self.exnref.0.get(ctx);
        let tag_handle = continuation.tag();
        crate::sys::tag::Tag {
    ...
    pub fn payload(&self, store: &mut impl AsStoreMut) -> Vec<Value> {
        ...
        let ctx = store.as_store_ref().objects().as_sys();
        let continuation = self.exnref.0.get(ctx);
        let params_ty = continuation.tag().get(ctx).signature.params().to_vec();
        let payload_ptr = continuation.payload();
...
impl loupe::MemoryUsage for Continuation {
