...
use crate::{
    ...
    error::RuntimeError,
    rt::sys::entities::engine::NativeEngineExt,
    vm::{VMExtern, VMExternTable},
    ExternRef, Function, RuntimeTable, Value,
};
...
impl crate::Table {
    /// Consume [`self`] into [`crate::rt::sys::table::Table`].
    pub fn into_sys(self) -> crate::rt::sys::table::Table {
        match self.0 {
            RuntimeTable::Sys(s) => s,
            _ => panic!("Not a `sys` table!"),
        }
    }

    /// Convert a reference to [`self`] into a reference [`crate::rt::sys::table::Table`].
    pub fn as_sys(&self) -> &crate::rt::sys::table::Table {
        match self.0 {
            RuntimeTable::Sys(ref s) => s,
            _ => panic!("Not a `sys` table!"),
        }
    }

    /// Convert a mutable reference to [`self`] into a mutable reference [`crate::rt::sys::table::Table`].
    pub fn as_sys_mut(&mut self) -> &mut crate::rt::sys::table::Table {
        match self.0 {
            RuntimeTable::Sys(ref mut s) => s,
            _ => panic!("Not a `sys` table!"),
        }
    }
}
