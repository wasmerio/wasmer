...
impl<'a, T> Into<crate::FunctionEnvMut<'a, T>> for FunctionEnvMut<'a, T> {
    fn into(self) -> crate::FunctionEnvMut<'a, T> {
        crate::RuntimeFunctionEnvMut::Jsc(self).into()
    }
}

impl<T> crate::FunctionEnv<T> {
    /// Consume [`self`] into [`crate::rt::jsc::function::env::FunctionEnv`].
    pub fn into_jsc(self) -> FunctionEnv<T> {
        match self.0 {
            crate::RuntimeFunctionEnv::Jsc(s) => s,
            _ => panic!("Not a `jsc` function env!"),
        }
    }

    /// Convert a reference to [`self`] into a reference to [`crate::rt::jsc::function::env::FunctionEnv`].
    pub fn as_jsc(&self) -> &FunctionEnv<T> {
        match self.0 {
            crate::RuntimeFunctionEnv::Jsc(ref s) => s,
            _ => panic!("Not a `jsc` function env!"),
        }
    }

    /// Convert a mutable reference to [`self`] into a mutable reference [`crate::rt::jsc::function::env::FunctionEnv`].
    pub fn as_jsc_mut(&mut self) -> &mut FunctionEnv<T> {
        match self.0 {
            crate::RuntimeFunctionEnv::Jsc(ref mut s) => s,
            _ => panic!("Not a `jsc` function env!"),
        }
    }
}
...
impl<'a, T> From<FunctionEnvMut<'a, T>> for crate::FunctionEnvMut<'a, T> {
    fn from(value: FunctionEnvMut<'a, T>) -> Self {
        crate::FunctionEnvMut(crate::RuntimeFunctionEnvMut::Jsc(value))
    }
}

impl<T> From<FunctionEnv<T>> for crate::FunctionEnv<T> {
    fn from(value: FunctionEnv<T>) -> Self {
        crate::FunctionEnv(crate::RuntimeFunctionEnv::Jsc(value))
    }
}
