...
use crate::{jsc::vm::VMMemory, vm::VMExtern, AsStoreMut, AsStoreRef, RuntimeMemory};
...
impl Memory {
    ...
    pub(crate) fn to_vm_extern(&self) -> VMExtern {
        VMExtern::Jsc(crate::rt::jsc::vm::VMExtern::Memory(self.handle.clone()))
    }
...
impl crate::Memory {
    /// Consume [`self`] into a [`crate::rt::jsc::mem::Memory`].
    pub fn into_jsc(self) -> crate::rt::jsc::memory::Memory {
        match self.0 {
            RuntimeMemory::Jsc(s) => s,
            _ => panic!("Not a `jsc` memory!"),
        }
    }

    /// Convert a reference to [`self`] into a reference to [`crate::rt::jsc::mem::Memory`].
    pub fn as_jsc(&self) -> &crate::rt::jsc::memory::Memory {
        match self.0 {
            RuntimeMemory::Jsc(ref s) => s,
            _ => panic!("Not a `jsc` memory!"),
        }
    }

    /// Convert a mutable reference to [`self`] into a mutable reference to [`crate::rt::jsc::mem::Memory`].
    pub fn as_jsc_mut(&mut self) -> &mut crate::rt::jsc::memory::Memory {
        match self.0 {
            RuntimeMemory::Jsc(ref mut s) => s,
            _ => panic!("Not a `jsc` memory!"),
        }
    }
}
