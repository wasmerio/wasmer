...
impl<T> crate::FunctionEnv<T> {
    /// Consume [`self`] into [`crate::rt::wasmi::function::env::FunctionEnv`].
    pub fn into_wasmi(self) -> FunctionEnv<T> {
        match self.0 {
            crate::RuntimeFunctionEnv::Wasmi(s) => s,
            _ => panic!("Not a `wasmi` function env!"),
        }
    }

    /// Convert a reference to [`self`] into a reference to [`crate::rt::wasmi::function::env::FunctionEnv`].
    pub fn as_wasmi(&self) -> &FunctionEnv<T> {
        match self.0 {
            crate::RuntimeFunctionEnv::Wasmi(ref s) => s,
            _ => panic!("Not a `wasmi` function env!"),
        }
    }

    /// Convert a mutable reference to [`self`] into a mutable reference [`crate::rt::wasmi::function::env::FunctionEnv`].
    pub fn as_wasmi_mut(&mut self) -> &mut FunctionEnv<T> {
        match self.0 {
            crate::RuntimeFunctionEnv::Wasmi(ref mut s) => s,
            _ => panic!("Not a `wasmi` function env!"),
        }
    }
}
...
impl<'a, T> From<FunctionEnvMut<'a, T>> for crate::FunctionEnvMut<'a, T> {
    fn from(value: FunctionEnvMut<'a, T>) -> Self {
        crate::FunctionEnvMut(crate::RuntimeFunctionEnvMut::Wasmi(value))
    }
}

impl<T> From<FunctionEnv<T>> for crate::FunctionEnv<T> {
    fn from(value: FunctionEnv<T>) -> Self {
        crate::FunctionEnv(crate::RuntimeFunctionEnv::Wasmi(value))
    }
}
