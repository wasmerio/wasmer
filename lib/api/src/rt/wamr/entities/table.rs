//! Data types, functions and traits for `wamr`'s `Table` implementation.
...
use crate::{
    ...
    AsStoreMut, AsStoreRef, RuntimeKind, RuntimeError, RuntimeTable, Value,
};

#[derive(Debug, Clone, PartialEq, Eq)]
/// A WebAssembly `table` in `wamr`.
pub struct Table {
...
impl Table {
    ...
    pub fn new(
        ...
        Ok(Self {
            handle: unsafe {
                wasm_table_new(
                    store_mut.inner.store.as_wamr().inner,
                    wasm_tablety,
                    init.of.ref_,
                )
            },
    ...
    pub fn to_vm_extern(&self) -> VMExtern {
        VMExtern::Wamr(unsafe { wasm_table_as_extern(self.handle) })
    }
    ...
    pub fn set(
        ...
        unsafe {
            let init = match val {
                ...
                Value::FuncRef(Some(ref r)) => wasm_func_as_ref(r.as_wamr().handle),
                _ => {
    ...
    pub fn grow(
        ...
        unsafe {
            ...
            let init = match init {
                Value::ExternRef(None) | Value::FuncRef(None) => std::ptr::null_mut(),
                Value::FuncRef(Some(r)) => wasm_func_as_ref(r.as_wamr().handle),
                _ => {
                    return Err(RuntimeError::new(format!(
...
impl crate::Table {
    /// Consume [`self`] into [`crate::rt::wamr::table::Table`].
    pub fn into_wamr(self) -> crate::rt::wamr::table::Table {
        match self.0 {
            RuntimeTable::Wamr(s) => s,
            _ => panic!("Not a `wamr` table!"),
        }
    }

    /// Convert a reference to [`self`] into a reference [`crate::rt::wamr::table::Table`].
    pub fn as_wamr(&self) -> &crate::rt::wamr::table::Table {
        match self.0 {
            RuntimeTable::Wamr(ref s) => s,
            _ => panic!("Not a `wamr` table!"),
        }
    }

    /// Convert a mutable reference to [`self`] into a mutable reference [`crate::rt::wamr::table::Table`].
    pub fn as_wamr_mut(&mut self) -> &mut crate::rt::wamr::table::Table {
        match self.0 {
            RuntimeTable::Wamr(ref mut s) => s,
            _ => panic!("Not a `wamr` table!"),
        }
    }
}
