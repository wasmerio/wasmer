...
/// The [`ModuleLike`] and [`ModuleCreator`] trait.
mod traits;
...
pub struct Module(pub(crate) Box<dyn ModuleLike>);
...
impl Module {
    ...
    pub fn from_binary(engine: &impl AsEngineRef, binary: &[u8]) -> Result<Self, CompileError> {
        Ok(Self(engine.as_engine_ref().engine().0.from_binary(binary)?))
    }
    ...
    pub unsafe fn deserialize_unchecked(
        ...
        Ok(Self(traits::ModuleCreator::deserialize_unchecked(
            engine.as_engine_ref().engine().0.as_ref(),
    ...
    pub unsafe fn deserialize(
        ...
        Ok(Self(traits::ModuleCreator::deserialize(
            engine.as_engine_ref().engine().0.as_ref(),
            bytes.into_bytes().into(),
        )?))
    }
    ...
    pub unsafe fn deserialize_from_file(
        ...
        Ok(Self(traits::ModuleCreator::deserialize_from_file(
            engine.as_engine_ref().engine().0.as_ref(),
            path.as_ref(),
        )?))
    }
    ...
    pub unsafe fn deserialize_from_file_unchecked(
        ...
        Ok(Self(
            traits::ModuleCreator::deserialize_from_file_unchecked(
                engine.as_engine_ref().engine().0.as_ref(),
                path.as_ref(),
            )?,
        ))
...
impl PartialEq for Module {
    fn eq(&self, other: &Self) -> bool {
        let r = self.0.cmp(other.0.as_ref());
        matches!(r, std::cmp::Ordering::Equal)
    }
}
