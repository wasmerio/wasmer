...
use crate::{
    buffer::{MemoryBuffer, MemoryBufferLike},
...
#[derive(Debug)]
pub struct MemoryView(pub(crate) Box<dyn MemoryViewLike>);

impl MemoryView {
    pub(crate) fn new(memory: &Memory, store: &(impl AsStoreRef + ?Sized)) -> Self {
        MemoryView(store.as_store_ref().memory_view_from_memory(memory))
    }
    ...
    #[inline]
    pub(crate) fn buffer(&self) -> MemoryBuffer {
...
/// The trait that every concrete memory view must implement.
pub trait MemoryViewLike: std::fmt::Debug {
    /// Returns the pointer to the raw bytes of the underlying memory.
    // This used by wasmer-c-api, but should be treated
    // as deprecated and not used in future code.
    #[doc(hidden)]
    ...
    #[inline]
    fn buffer(&self) -> Box<dyn MemoryBufferLike>;
    ...
    /// This method is guaranteed to be safe (from the host side) in the face of
    /// concurrent writes.
    fn read_u8(&self, offset: u64) -> Result<u8, MemoryAccessError>;
    ...
    /// This method is guaranteed to be safe (from the host side) in the face of
    /// concurrent writes.
    fn write_u8(&self, offset: u64, val: u8) -> Result<(), MemoryAccessError>;
}

/// The trait implemented by all those that can create new globals.
pub trait MemoryViewCreator {
    fn memory_view_from_memory(&self, memory: &Memory) -> Box<dyn MemoryViewLike>;
}
