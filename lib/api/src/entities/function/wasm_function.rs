...
use crate::{
    error::RuntimeError, vm::VMFuncRef, AsStoreMut, AsStoreRef, ExportError, Exportable, Extern,
    FunctionEnv, StoreRef, Value,
};
...
impl Function {
    ...
    pub(crate) fn vm_funcref(&self, store: &impl AsStoreRef) -> VMFuncRef {
        self.0.vm_funcref(store.as_store_ref())
    }
    //
...
pub trait FunctionLike: std::fmt::Debug { 
    fn vm_funcref(&self, store: StoreRef) -> VMFuncRef;
