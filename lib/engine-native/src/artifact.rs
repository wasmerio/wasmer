...
impl NativeArtifact {
    ...
    pub unsafe fn deserialize(engine: &NativeEngine, bytes: &[u8]) -> Result<NativeArtifact, DeserializeError> {
    ...
    pub unsafe fn deserialize_from_file_unchecked(
        ...
        let symbol: Symbol<*mut [u8; 10 + 1]> = lib.get(b"WASMER_METADATA").map_err(|e| {
            DeserializeError::CorruptedBinary(format!("The provided object file doesn't seem to be generated by Wasmer: {}", e))
        })?;
        use std::ops::Deref;
        use std::slice;
