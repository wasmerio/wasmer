...
fn c7_l105_action_invoke(instance: &mut Instance) -> Result<(), String> {
    ...
    assert_eq!(result, Ok(Some(Value::F32((11.1f32).to_bits()))));
    result.map(|_| ())
}
...
fn c8_l106_action_invoke(instance: &mut Instance) -> Result<(), String> {
    ...
    assert_eq!(result, Ok(Some(Value::F64((12.2f64).to_bits()))));
    result.map(|_| ())
}
...
fn c11_l122_action_invoke(instance: &mut Instance) -> Result<(), String> {
    ...
    let result = instance.call("result", &[Value::I64(-1 as i64), Value::F32((-2.0f32).to_bits()), Value::F64((-3.3f64).to_bits()), Value::I32(-4 as i32), Value::I32(-5 as i32)]);
