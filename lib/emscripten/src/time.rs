...
pub fn _gettimeofday(ctx: FunctionEnvMut<EmEnv>, tp: c_int, tz: c_int) -> c_int {
    ...
    unsafe {
        ...
        let timeval_struct_ptr =
            emscripten_memory_pointer!(ctx.data().memory_view(0, &ctx), tp) as *mut GuestTimeVal;
...
pub fn _clock_gettime(ctx: FunctionEnvMut<EmEnv>, clk_id: clockid_t, tp: c_int) -> c_int {
    ...
    unsafe {
        let timespec_struct_ptr =
            emscripten_memory_pointer!(ctx.data().memory_view(0, &ctx), tp) as *mut GuestTimeSpec;
...
pub fn _localtime_r(ctx: FunctionEnvMut<EmEnv>, time_p: u32, result: u32) -> c_int {
    ...
    unsafe {
        ...
        let result_addr =
            emscripten_memory_pointer!(ctx.data().memory_view(0, &ctx), result) as *mut guest_tm;
...
pub fn _timegm(ctx: FunctionEnvMut<EmEnv>, time_ptr: u32) -> i32 {
    ...
    unsafe {
        let time_p_addr =
            emscripten_memory_pointer!(ctx.data().memory_view(0, &ctx), time_ptr) as *mut guest_tm;
