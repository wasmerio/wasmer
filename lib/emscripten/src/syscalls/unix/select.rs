...
#[cfg(feature = "vfs")]
fn translate_to_host_file_descriptors(ctx: &mut Ctx, mut varargs: &mut VarArgs, nfds: i32, fds_set_offset: u32) -> (i32, HashMap<i32, i32>, Vec<FdPair>) {
...
//    let ofds = (0..nfds).filter_map(|v| fd_slice.v)
    ...
    let file_descriptor_pairs = original_fds.iter()
        .filter(|vfd| {
...
//        if pair.virtual_fd > sz { sz = pair.host_fd }
        if pair.host_fd > sz { sz = pair.host_fd }
...
#[cfg(feature = "vfs")]
fn translate_to_virtual_file_descriptors(ctx: &mut Ctx, nfds: i32, fds_set_offset: u32, lookup: HashMap<i32, i32>) -> Vec<FdPair> {
    ...
    let pairs = fds.iter()
        .filter_map(|host_fd| {
            lookup.get(&host_fd).map(|virtual_fd| (*virtual_fd, host_fd))
        })
...
//    debug!("select write descriptors: {:?}", write_fds);
    ...
    let max = if read_max > write_max { read_max } else { write_max };

    let mut sz = max;

    debug!("set read descriptors BEFORE select: {:?}", read_pairs);
    debug!("set write descriptors BEFORE select: {:?}", write_pairs);
    ...
    debug!("select read descriptors after select completes: {:?}", read_pairs);

    let write_pairs = translate_to_virtual_file_descriptors(ctx, sz, writefds, write_lookup);
    debug!("select write descriptors after select completes: {:?}", write_pairs);
