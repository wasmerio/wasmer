...
mod tests {
    ...
    fn errors_for_unsupported_targets() {
        ...
        let result = compiler.compile_module(&linux32, &info, &translation, inputs, None, false);
        ...
        let (info, translation, inputs) = dummy_compilation_ingredients();
        let result = compiler.compile_module(&win32, &info, &translation, inputs, None, false);
        match result.unwrap_err() {
