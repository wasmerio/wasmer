...
impl Machine for MachineX86_64 {
    ...
    fn gen_std_dynamic_import_trampoline(
        ...
        match calling_convention {
            ...
            _ => {
                println!("trampoline: load target address = vmoffsets.vmdynamicfunction_import_context_address(): {:?}", vmoffsets.vmdynamicfunction_import_context_address());
                // Load target address.
                a.emit_mov(
                    Size::S64,
                    Location::Memory(
        ...
        let function_body = a.finalize().unwrap().to_vec();

        println!("trampoline function assembler code:\r\n\r\n{}\r\n", disassemble_x86_64_debug(&function_body));
    ...
    fn gen_import_call_trampoline(
        ...
        println!("getn import call trampoline");
...
fn disassemble_x86_64_debug(bytes: &[u8]) -> String {
    ...
    while decoder.can_decode() {
        ...
        real_output.push_str(&format!("{:016X} ", instruction.ip()));
        let start_index = (instruction.ip() - EXAMPLE_CODE_RIP) as usize;
        let instr_bytes = &bytes[start_index..start_index + instruction.len()];
        for b in instr_bytes.iter() {
            real_output.push_str(&format!("{:02X}", b));
        }
        if instr_bytes.len() < HEXBYTES_COLUMN_BYTE_LENGTH {
            for _ in 0..HEXBYTES_COLUMN_BYTE_LENGTH - instr_bytes.len() {
                real_output.push_str(&format!("  "));
            }
        }
        real_output.push_str(&format!(" {}\r\n", output));
    }
