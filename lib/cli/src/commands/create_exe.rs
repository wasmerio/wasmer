...
use pirita::{PiritaFileMmap, ParseOptions};
...
impl CreateExe {
    ...
    pub fn execute(&self) -> Result<()> {
        ...
        #[cfg(feature = "pirita_file")] {
            if let Ok(pirita) = PiritaFileMmap::parse(wasm_module_path.clone(), &ParseOptions::default()) {
    ...
    fn create_exe_pirita(&self, file: &PiritaFileMmap, target: Target) -> anyhow::Result<()> {
        ...
        let compiled_modules = file.get_all_atoms()
            .into_iter()
        ...
        volumes_object.write_stream(&mut writer)
            .map_err(|err| anyhow::anyhow!(err.to_string()))?;
        writer.flush()?;
        drop(writer);
