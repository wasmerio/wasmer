...
mod tests {
    use super::*;
    use crate::commands::{Cmd, WasmerCmd};
    use clap::Parser;

    #[test]
    fn create_namespace_command_parses() {
        let parsed = WasmerCmd::try_parse_from([
            "wasmer",
            "namespace",
            "create",
            "my-namespace",
        ...
        let Some(Cmd::Namespace(namespace)) = parsed.cmd else {
            panic!("expected namespace command");
        };

        let crate::commands::namespace::CmdNamespace::Create(create) = namespace else {
            panic!("expected namespace create command");
        };

        assert_eq!(create.name.as_deref(), Some("my-namespace"));
        assert_eq!(create.display_name.as_deref(), Some("My Namespace"));
        assert_eq!(create.description.as_deref(), Some("Namespace description"));
        assert!(create.non_interactive);
    }

    #[test]
    fn missing_name_errors_in_non_interactive_mode() {
        let cmd = CmdNamespaceCreate {
        ...
        let error = cmd.get_name().unwrap_err();
        assert_eq!(
            error.to_string(),
            "No namespace name given. Provide one as a positional argument."
        );
    }

    #[test]
    fn empty_optional_values_are_ignored() {
        assert_eq!(CmdNamespaceCreate::normalize_optional_value(Some("")), None);
        assert_eq!(CmdNamespaceCreate::normalize_optional_value(Some("   ")), None);
        assert_eq!(
            CmdNamespaceCreate::normalize_optional_value(Some("  hello  ")),
