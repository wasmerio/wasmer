...
/// Apply the compiled code's relocations to `section`, declaring the
/// referenced symbols. Relocations against the function itself resolve to
/// `local_symbol`; other local functions, import trampolines (custom
/// sections) and libcalls are declared as undefined symbols resolved at link
/// or load time.
    ...
    for relocation in relocations {
        let symbol = match relocation.reloc_target {
            ...
            RelocationTarget::DynamicTrampoline(index) => {
                add_undefined_symbol(object, format!("dt{}", index.as_u32()))
            }
        };
...
/// Serialize an import trampoline's custom section into its own relocatable
/// object file, under an `i{index}` symbol.
