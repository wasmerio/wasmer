...
use wasmparser::{
    ...
    MemoryType as WPMemoryType, NameSectionReader, Naming, Operator, TableSectionReader, TypeRef,
    TypeSectionReader,
};
...
pub fn wptype_to_type(ty: wasmparser::ValType) -> WasmResult<Type> {
    match ty {
        ...
        wasmparser::ValType::Ref(reftype) => wpreftype_to_type(reftype),
    }
}

/// Helper function translating wasmparser types to Wasm Type.
pub fn wpreftype_to_type(ty: wasmparser::RefType) -> WasmResult<Type> {
    wpheaptype_to_type(ty.heap_type)
}
    ...
    for entry in types {
        if let Ok(wasmparser::Type::Func(functype)) = entry {
            ...
            module_translation_state
                .wasm_types
                .push((params.to_vec(), returns.to_vec()));
        } else {
...
fn read_elems(items: &ElementItems) -> WasmResult<Box<[FunctionIndex]>> {
    ...
    match items {
        ElementItems::Expressions(section) => {
            let sections = section.clone().into_iter_with_offsets();
            for item in sections {
                let (_item_size, item) = item.map_err(from_binaryreadererror_wasmerror)?;
                let mut item_reader = item.get_operators_reader();
                let elem = match item_reader.read() {
                    Ok(Operator::RefNull { .. }) => FunctionIndex::reserved_value(),
                    Ok(Operator::RefFunc { function_index }) => {
        ...
        ElementItems::Functions(section) => {
            let sections = section.clone().into_iter_with_offsets();
            for item in sections {
                let (_size, index) = item.map_err(from_binaryreadererror_wasmerror)?;
                let elem = FunctionIndex::from_u32(index);
                elems.push(elem);
            }
        }
    }
...
pub fn parse_element_section<'data>(
    ...
    for (index, entry) in elements.into_iter().enumerate() {
        ...
        if ty.heap_type != wasmparser::HeapType::Func {
            return Err(wasm_unsupported!(
    ...
    while let Some(Ok(subsection)) = names.next() {
        match subsection {
            wasmparser::Name::Function(function_subsection) => {
                for naming in function_subsection.into_iter_with_offsets() {
                    if let Some((index, name)) = parse_function_name_subsection(naming) {
...
fn parse_function_name_subsection(
    naming: Result<(usize, Naming<'_>), wasmparser::BinaryReaderError>,
) -> Option<(FunctionIndex, &str)> {
    if naming.is_err() {
        return None;
    }
    let naming = naming.unwrap().1;
    let index = naming.index;
    let name = naming.name;
    if index == std::u32::MAX {
        // We reserve `u32::MAX` for our own use.
        return None;
    }
    Some((FunctionIndex::from_u32(index), name))
}
