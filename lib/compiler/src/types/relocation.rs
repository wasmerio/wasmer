...
use wasmer_types::{entity::PrimaryMap, lib::std::fmt, FunctionIndex, LibCall, LocalFunctionIndex};
...
pub enum RelocationTarget {
    ...
    /// An invalid relocation target; this is useful for things that shouldn't be the
    /// target of a relocation, such as function call trampolines so we'll know to
    /// error out if we encounter them.
    Invalid,
}
