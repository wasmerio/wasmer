...
pub struct InfoCollection<'wasm> {
    reader: ModuleReader<'wasm>,
}

impl<'wasm> InfoCollection<'wasm> {
    pub fn new(wasm: &'wasm [u8]) -> Result<Self, BinaryReaderError> {
        Ok(Self {
            reader: ModuleReader::new(wasm)?,
        })
    }
                    ...
                    for export in export_reader {
                        ...
                        let export_index = match kind {
                            ExternalKind::Function => {
                                ExportIndex::Func(FuncIndex::new(index as usize))
                            }
                            ExternalKind::Table => {
                                ExportIndex::Table(TableIndex::new(index as usize))
                            }
                            ExternalKind::Memory => {
...
fn type_to_type(ty: WpType) -> Result<Type, BinaryReaderError> {
    Ok(match ty {
        WpType::I32 => Type::I32,
        WpType::I64 => Type::I64,
        WpType::F32 => Type::F32,
