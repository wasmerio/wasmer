...
void test_main(void) {
    ...
    wasix_context_new(&context2, test1);
    ...
    // Assert that calling the deleteed context again fails
    wasix_context_delete(context1);
    wasix_context_switch(context1);
    assert(counter == 1);

    // Assert that switching to a context that was deleteed before the first switch works
    wasix_context_delete(context2);
...
int main() {
    ...
    wasix_context_new(&test_main_context, test_main);
