...
int main() {
  ...
  int seen_efd2 = 0;
  for (int i = 0; i < n; i++) {
    if (events[i].data.fd == efd1) seen_efd1 = 1;
    if (events[i].data.fd == efd2) seen_efd2 = 1;
  }
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (events[i].data.fd == efd2) {
      if ((events[i].events & EPOLLOUT) != 0) seen_out = 1;
  ...
  if (efd1 != -1) close(efd1);
  if (efd2 != -1) close(efd2);
