...
//#ExpectedStdoutFile: ported_path_rename.stdout
// WASI:
// tempdir: temp
...
fn run_with_sub_dir() {
    ...
    std::fs::rename(&file_to_create, &file_to_rename_to).unwrap();
    let _file = fs::File::open(&file_to_rename_to).expect("Could not open file");
    if file_to_create.exists() {
    ...
    fs::remove_dir_all(base.join("sub")).unwrap();
}
...
fn run_with_different_sub_dirs() {
    ...
    fs::create_dir(base.join("a/b")).unwrap();
    fs::create_dir(base.join("c")).unwrap();
    fs::create_dir(base.join("c/d")).unwrap();
    fs::create_dir(base.join("c/d/e")).unwrap();
    ...
    fs::remove_dir_all(base.join("c")).unwrap();
}
