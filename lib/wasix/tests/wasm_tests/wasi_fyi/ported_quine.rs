...
//#ExpectedStdoutFile: ported_quine.stdout
// WASI:
// dir: test_fs
...
fn main() {
    let mut this_file = fs::File::open("/hamlet/act1/scene2.txt").expect("could not find src file");
    let _ = this_file.metadata().unwrap();
