...
//#ExpectedStdoutFile: inode.stdout
...
fn main() {
    ...
    {
        let meta1 =
            fs::metadata("test_fs/hamlet/act1/scene1.txt").expect("could not find src file");
        let meta2 =
            fs::metadata("test_fs/hamlet/act1/scene2.txt").expect("could not find src file");
        if meta1.dev() == meta2.dev() && meta1.ino() == meta2.ino() {
            println!("Warning, different files from same folder have same dev/inod");
        }
        let meta3 =
            fs::metadata("test_fs/hamlet/act2/scene1.txt").expect("could not find src file");
        ...
        let meta4 = fs::metadata("test_fs/hamlet/act1/../act1/scene1.txt")
