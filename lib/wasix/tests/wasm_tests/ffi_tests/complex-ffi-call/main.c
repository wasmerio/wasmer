...
int sum_ten(int a, int b, int c, int d, int e, int f, int g, int h, int i,
            int j) {
  return a + b + c + d + e + f + g + h + i + j;
}

// Test 3: Long double operations
long double multiply_ldoubles(long double a, long double b) { return a * b; }
...
int main() {
  ...
  // Test 1: Struct passing and returning
  {
    ...
    status = ffi_prep_cif(&struct_cif, FFI_DEFAULT_ABI, 1, &struct_type,
                          struct_arg_types);
    if (status != FFI_OK) {
    ...
    printf("TestStruct size: %zu, alignment: %zu\n", sizeof(TestStruct),
           __alignof__(TestStruct));
    printf("TestStruct field offsets: x=%zu, y=%zu, z=%zu\n",
           (size_t)&((TestStruct*)0)->x, (size_t)&((TestStruct*)0)->y,
    ...
    ffi_call(&struct_cif, (void (*)(void))opaque_update_struct, &test_out,
             args);
  ...
  // Test 2: 10 arguments
  {
    ...
    status = ffi_prep_cif(&ten_args_cif, FFI_DEFAULT_ABI, 10, &ffi_type_sint32,
                          ten_args_types);
    if (status != FFI_OK) {
    ...
    int result;
    ffi_call(&ten_args_cif, (void (*)(void))opaque_sum_ten, &result,
             arg_values);
  ...
  // Test 3: Long double operations
  {
    ...
    status = ffi_prep_cif(&ld_cif, FFI_DEFAULT_ABI, 2, &ffi_type_longdouble,
                          ld_arg_types);
    if (status != FFI_OK) {
    ...
    void* ld_args[2] = {&a, &b};
    ffi_call(&ld_cif, (void (*)(void))opaque_multiply_ldoubles, &result,
             ld_args);
  ...
  // Test 4: Return a struct from a function
  {
    ...
    status = ffi_prep_cif(&create_cif, FFI_DEFAULT_ABI, 3, &return_struct_type,
                          create_arg_types);
    if (status != FFI_OK) {
    ...
    TestStruct result;
    ffi_call(&create_cif, (void (*)(void))opaque_create_struct, &result,
             create_args);
