...
int run_tests() {
  ...
  // Open zzz on 11
  int fd = open("./output.zzz.tmp", O_WRONLY | O_CREAT, 0);
  if (fd < 0) {
    perror("open");
    return 1;
  }
  ...
  char buffer[5] = {0};
  fd = open("./output.yyy.tmp", O_RDONLY | O_CREAT, 0);
  if (fd < 0) {
    perror("open");
    return 1;
  }
  ...
  fd = open("./output.zzz.tmp", O_RDONLY | O_CREAT, 0);
  if (fd < 0) {
    perror("open");
    return 1;
  }
  ...
  if (posix_spawn_file_actions_init(&fdops) != 0) {
    perror("posix_spawn_file_actions_init 2");
    return 1;
  }
  if (posix_spawnattr_init(&attr) != 0) {
    perror("posix_spawnattr_init 2");
    return 1;
  }

  if (posix_spawnp(&pid, "main-not-asyncified.wasm", &fdops, &attr, argv2,
                   envp2) != 0) {
  ...
  if (posix_spawn_file_actions_destroy(&fdops) != 0) {
    perror("posix_spawn_file_actions_destroy 2");
    return 1;
  }
  if (posix_spawnattr_destroy(&attr) != 0) {
    perror("posix_spawnattr_destroy 2");
    return 1;
  }
...
void write_subprocess_error(const char* msg) {
  FILE* outf = fopen("./output.child.tmp", "w");
  if (!outf) {
    exit(EXIT_FAILURE);
  }
  fprintf(outf, "%s: %s\n", msg, strerror(errno));
