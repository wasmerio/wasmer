...
pub trait Callbacks<M>: Send + Sync + 'static
where
    M: Send + Sync + 'static,
{
    ...
    async fn recycle_env(&self, conf: RecycleEnvConfig<M>) {
        default_recycle_env(conf).await
    }

    /// Create the WASI environment
    async fn create_env(&self, conf: CreateEnvConfig<M>) -> anyhow::Result<CreateEnvResult> {
