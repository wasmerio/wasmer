...
impl SnapshotEffector {
    pub fn save_fd_pipe(
        ...
        Self::save_event(ctx, SnapshotLog::CreatePipe { fd1, fd2 })
    }
    ...
    pub fn apply_fd_pipe(
        ...
        let (ret_fd1, ret_fd2) = crate::syscalls::fd_pipe_internal(ctx).map_err(|err| {
            anyhow::format_err!("snapshot restore error: failed to create pipe - {}", err)
        })?;

        let ret = crate::syscalls::fd_renumber_internal(ctx, ret_fd1, fd1);
        if ret != Errno::Success {
            bail!(
                "snapshot restore error: failed renumber file descriptor after create pipe (from={}, to={}) - {}",
                ret_fd1,
                fd1,
                ret
            );
