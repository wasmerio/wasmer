...
async fn fetch_webc(
    ...
    pkg.version = semver::Version::parse(&version).unwrap();
    Ok(pkg)
}
...
fn parse_webc_v2(webc: &Container) -> Result<BinaryPackage, anyhow::Error> {
    ...
    let pkg = BinaryPackage {
        ...
        uses,
        version: semver::Version::parse(&wapm.version).unwrap(),
...
mod tests {
    ...
    fn parse_a_webc_with_multiple_commands() {
        ...
        assert_eq!(pkg.version, semver::Version::parse("1.0.14").unwrap());
        assert_eq!(pkg.uses, Vec::<String>::new());
    ...
    fn parse_a_webc_with_dependencies() {
        ...
        assert_eq!(pkg.version, semver::Version::parse("1.0.12").unwrap());
