...
use futures::Future;
use tracing::*;
use wasmer::{Function, FunctionEnvMut, Module, Store};
use wasmer_types::{Memory32, Memory64};
...
pub fn spawn_exec(
    ...
    let module = compiled_modules.get_compiled_module(&store, binary.hash(), compiler);
    let module = match (module, binary.entrypoint_cmd.as_ref()) {
        (Some(a), _) => a,
        (None, Some(entry)) => {
            ...
            let module = module?;
            compiled_modules.set_compiled_module(binary.hash(), compiler, &module);
            module
        }
        (None, None) => {
