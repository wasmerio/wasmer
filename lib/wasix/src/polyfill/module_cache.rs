...
use super::{wapm, BinaryPackage};
use crate::{runtime::module_cache::ModuleHash, syscalls::platform_clock_time_get, Runtime};
...
pub struct ModuleCache {
    ...
    pub(crate) cached_modules: Option<RwLock<HashMap<ModuleHash, Module>>>,
...
thread_local! {
    static THREAD_LOCAL_CACHED_MODULES: std::cell::RefCell<HashMap<ModuleHash, Module>>
...
impl ModuleCache {
    ...
    pub fn get_webc(&self, webc: &str, runtime: &dyn Runtime) -> Option<BinaryPackage> {
    ...
    fn get_webc_slow(
        ...
        // Now try for the WebC
        {
            ...
            if let Ok(mut data) = wapm::fetch_webc_task(&self.cache_webc_dir, wapm_name, runtime) {
                ...
                while data.entrypoint_cmd.is_none() {
                    ...
                    if let Some(inherits) = inherits.next() {
                        data.entrypoint_cmd = inherits.entrypoint_cmd.clone();
                    } else {
                        break;
                    }
                }
    ...
    pub fn get_compiled_module(
        ...
        data_hash: ModuleHash,
        compiler: &str,
    ) -> Option<Module> {
        let key = ModuleHash::sha256(format!("{}-{}", data_hash, compiler).as_bytes());
        ...
        if let Ok(data) = std::fs::read(path.as_path()) {
            ...
            if let Ok(data) = decoder.decode(&data[..]) {
                ...
                // Load the module
                let module = unsafe {
                    match Module::deserialize_unchecked(engine, &module_bytes[..]) {
                        Ok(m) => m,
                        Err(err) => {
                            tracing::error!(
    ...
    pub fn set_compiled_module(&self, data_hash: ModuleHash, compiler: &str, module: &Module) {
        let key = ModuleHash::sha256(format!("{}-{}", data_hash, compiler).as_bytes());
...
mod tests {
    ...
    use crate::{
        runtime::{resolver::WapmSource, task_manager::tokio::TokioTaskManager},
        PluggableRuntime,
    };
