...
static CANARY: u8 = 0;
const WASMER_EXCEPTION_CLASS: libunwind::_Unwind_Exception_Class =
    u64::from_ne_bytes(*b"WER\0WASM");
...
pub unsafe extern "C" fn wasmer_eh_personality(
    ...
    unsafe {
        ...
        let rust_exc = std::mem::transmute::<*mut uw::_Unwind_Exception, *mut UwExceptionWrapper>(
            exception_object,
        );

        let wasmer_exc = (*rust_exc).cause.downcast_ref::<WasmerException>();
        let wasmer_exc = wasmer_exc.unwrap();
        let wasmer_exc_ptr = wasmer_exc as *const _ as usize;
        ...
        if actions as i32 & uw::_Unwind_Action__UA_SEARCH_PHASE as i32 != 0 {
            ...
            match eh_action {
                ...
                EHAction::Cleanup(lpad) => {
                    uw::_Unwind_SetGR(context, UNWIND_DATA_REG.0, wasmer_exc_ptr as u64);
