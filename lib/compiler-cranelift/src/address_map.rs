...
use wasmer_compiler::{
    FunctionAddressMap, InstructionAddressMap, MiddlewareBinaryReader, SourceLoc,
};

pub fn get_function_address_map<'data>(
    context: &Context,
    data: &MiddlewareBinaryReader<'data>,
    body_len: usize,
    ...
    let start_srcloc = SourceLoc::new(data.original_position() as u32);
    let end_srcloc = SourceLoc::new((data.original_position() + data.len()) as u32);
