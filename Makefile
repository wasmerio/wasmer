...
test-all-windows:
	...
	$(CARGO_BINARY) nextest run $(CARGO_TARGET_FLAG) --package wasmer-compiler --release --locked && \
	$(CARGO_BINARY) nextest run $(CARGO_TARGET_FLAG) --package wasmer-wasix --release --features v8 --locked && \
	$(CARGO_BINARY) test --doc $(CARGO_TARGET_FLAG) --package wasmer --release --no-default-features --features v8-default --locked
