...
build-wasmer:
	c++ --version && which c++ && CC="$(xcrun -f clang)" CXX="$(xcrun -f clang++)" $(CARGO_BINARY) build $(CARGO_TARGET_FLAG) --manifest-path lib/cli/Cargo.toml $(build_compiler_features) --bin wasmer --locked
