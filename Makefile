...
test-build-docs-rs-ci:
	...
	for manifest_path in lib/*/Cargo.toml; do \
		...
		printf "*** Building doc for package with manifest $$manifest_path ***\n\n"; \
		if [ -z "$$features" ]; then \
			RUSTDOCFLAGS="--cfg=docsrs" $(CARGO_BINARY) +nightly-2025-01-10  doc $(CARGO_TARGET_FLAG) --manifest-path "$$manifest_path" --no-deps --locked || exit 1; \
		else \
			printf "Following features are inferred from Cargo.toml: $$features\n\n\n"; \
			RUSTDOCFLAGS="--cfg=docsrs" $(CARGO_BINARY) +nightly-2025-01-10 doc $(CARGO_TARGET_FLAG) --manifest-path "$$manifest_path" --no-deps --features "$$features" --locked || exit 1; \
		fi; \
	done
