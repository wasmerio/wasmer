...
static void test_invalid_pointer(void)
{
    ...
    __wasi_errno_t err = __wasi_port_route_remove(bad_ptr);
...
static void test_invalid_tag(void)
{
    ...
    __wasi_errno_t err = __wasi_port_route_remove(&addr);
...
static void test_host_net_not_supported(void)
{
    ...
    __wasi_errno_t err = __wasi_port_route_remove(&addr);
    // NOTE: host networking backend does not allow mutating routes.
    assert(err == __WASI_ERRNO_NOTSUP);
}

int main(void)
{
    printf("WASIX port_route_remove integration tests\n");
