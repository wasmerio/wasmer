...
{
    printf("Test 1: wake_all with no waiters returns false\n");
...
{
    printf("Test 2: wake_all wakes all waiters\n");
    enum { kThreads = 4 };
...
static void test_after_drain(void)
{
    printf("Test 3: wake_all after draining returns false\n");
    __wasi_bool_t woken = __WASI_BOOL_TRUE;
    __wasi_errno_t err = __wasi_futex_wake_all((uint32_t *)&futex_word, &woken);
    assert(err == __WASI_ERRNO_SUCCESS);
    assert(woken == __WASI_BOOL_FALSE);
