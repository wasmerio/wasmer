...
fn native_host_function_closure_panics(config: crate::Config) {
    ...
    let mut ctx = FunctionEnv::new(&mut store, ());
    let state = 3;
    Function::new_native(&mut store, &ctx, move |_ctx: ContextMut<_>, _: i32| {
        println!("{}", state);
    });
}
...
fn native_with_env_host_function_closure_panics(config: crate::Config) {
    ...
    let state = 3;
    Function::new_native(&mut store, &ctx, move |_ctx: ContextMut<i32>, _: i32| {
        println!("{}", state);
    });
}
...
fn non_native_functions_and_closures_with_no_env_work(config: crate::Config) -> anyhow::Result<()> {
    ...
    let import_object = imports! {
        "env" => {
            ...
            "multiply3" => Function::new_native(&mut store, &ctx, |_ctx: ContextMut<_>, arg1: i32, arg2: i32| -> i32
                                                {arg1 * arg2 }),
            "multiply4" => Function::new_native(&mut store, &ctx, |ctx: ContextMut<i32>, arg1: i32, arg2: i32| -> i32
                                                         {arg1 * arg2 * ctx.data() }),
        },
    };
