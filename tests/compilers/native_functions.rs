...
fn native_function_works_for_wasm(config: crate::Config) -> anyhow::Result<()> {
    ...
    let import_object = imports! {
        "env" => {
            "multiply" => Function::new_native(&mut store, &ctx, |_ctx: FunctionEnvMut<_>, a: i32, b: i32| a * b),
        },
    };
...
fn native_host_function_closure_panics(config: crate::Config) {
    ...
    let mut ctx = FunctionEnv::new(&mut store, ());
    let state = 3;
    Function::new_native(&mut store, &ctx, move |_ctx: FunctionEnvMut<_>, _: i32| {
        println!("{}", state);
    });
}
...
fn non_native_functions_and_closures_with_no_env_work(config: crate::Config) -> anyhow::Result<()> {
    ...
    let import_object = imports! {
        "env" => {
            "multiply1" => Function::new(&mut store, &ctx, &ty, move |_ctx, args| {
            ...
            "multiply2" => Function::new(&mut store, &ctx, &ty, move |ctx, args| {
                if let (Value::I32(v1), Value::I32(v2)) = (&args[0], &args[1]) {
            ...
            "multiply3" => Function::new_native(&mut store, &ctx, |_ctx: FunctionEnvMut<_>, arg1: i32, arg2: i32| -> i32
                                                {arg1 * arg2 }),
            "multiply4" => Function::new_native(&mut store, &ctx, |ctx: FunctionEnvMut<i32>, arg1: i32, arg2: i32| -> i32
                                                         {arg1 * arg2 * ctx.data() }),
        },
    };
...
fn native_function_works_for_wasm_function_manyparams(config: crate::Config) -> anyhow::Result<()> {
    ...
    let import_object = imports! {
        "env" => {
            "longf" => Function::new_native(&mut store, &ctx, long_f),
        },
    };
...
fn native_function_works_for_wasm_function_manyparams_dynamic(
    ...
    let import_object = imports! {
        "env" => {
            "longf" => Function::new(&mut store, &ctx, FunctionType::new(vec![ValueType::I32, ValueType::I32, ValueType::I32, ValueType::I32, ValueType::I32, ValueType::I32, ValueType::I64 , ValueType::I64 ,ValueType::I32, ValueType::I32], vec![ValueType::I64]), long_f_dynamic),
        },
    };
...
fn static_host_function_without_env(config: crate::Config) -> anyhow::Result<()> {
    ...
    {
        let f = Function::new_native(&mut store, &ctx, f);
    ...
    // Native static host function that returns a tuple.
    {
        let long_f = Function::new_native(&mut store, &ctx, long_f);
    ...
    {
        let f = Function::new_native(&mut store, &ctx, f_ok);
        let f_native: TypedFunction<(i32, i64, f32, f64), (f64, f32, i64, i32)> =
            f.native(&mut store).unwrap();
        let result = f_native.call(&mut store, 1, 3, 5.0, 7.0)?;
        assert_eq!(result, (28.0, 15.0, 6, 1));
...
fn static_host_function_with_env(config: crate::Config) -> anyhow::Result<()> {
    ...
    // Native static host function that returns a tuple.
    {
        ...
        assert_eq!(*ctx.as_mut(&mut store).0.lock().unwrap(), 100);

        let result = f_native.call(&mut store, 1, 3, 5.0, 7.0)?;

        assert_eq!(result, (28.0, 15.0, 6, 1));
        assert_eq!(*ctx.as_mut(&mut store).0.lock().unwrap(), 101);
    }
