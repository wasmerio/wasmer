 ...
 *
 * When a symlink's target is deleted, os.path.exists() (stat with
 * follow_symlinks) must return false.  The bug was that get_inode_at_path_inner
 * returned the cached symlink inode directly instead of continuing symlink
 * resolution, so stat() succeeded even for a dangling symlink.
 *
 */
