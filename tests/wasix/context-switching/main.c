...
int vfork_after_switching2() {
  // Create two contexts. One is to verify switching works now and the other to
  // verify it works later on.
  ...
  assert(was_in_context_fn_switch_to_main == 1 &&
         "Context function was not executed");

  int pid = vfork();

  if (pid == 0) {
    // The process created by vfork should not have a context switching
    // environment so we get a ENOSYS here
...
int fork_after_switching() {
  ...
  if (pid == 0) {
    // The process created by fork should have a new context switching
    // environment so we get a EINVAL here because the context does not exist in
...
int posix_spawning_a_forking_subprocess_from_a_context() {
  wasix_context_id_t ctx;
  int ret = wasix_context_create(
      &ctx, context_fn_posix_spawn_a_forking_subprocess_from_a_context);
  assert(ret == 0 && "Failed to create context");
  ret = wasix_context_switch(ctx);
  assert(ret == 0 && "Failed to switch to context");
  return 0;
