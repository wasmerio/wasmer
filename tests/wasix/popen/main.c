...
// For the tests we store the pid in a global instead of the file as wasix-libc
// does not expose FILE internals. This does not change the behaviour as long as
// there is only a single popen at a time, which is the case for our tests.
static pid_t g_popen_pid = -1;

// Exact copy of wasix-libc's popen implementation, except using the global pid
// instead of f->pipe_pid Also no calls to FLOCK and FUNLOCK, but those are
// no-ops in wasix-libc anyway so that doesn't change behavior
FILE* mypopen(const char* cmd, const char* mode) {
  ...
  if (!posix_spawn_file_actions_init(&fa)) {
    if (!posix_spawn_file_actions_adddup2(&fa, p[1 - op], 1 - op)) {
      ...
      if (!(e = posix_spawn(
                &pid, "./main.wasm", &fa, 0,
                (char*[]){"./main.wasm", "shell", "-c", (char*)cmd, 0},
                environ))) {
...
// Exact copy of wasix-libc's pclose implementation, except using the global pid
// instead of f->pipe_pid
int mypclose(FILE* f) {
  int status, r;
...
int main(int argc, char* argv[]) {
  if (argc < 2) {
    ...
    fprintf(
        stderr,
        "Commands: echo, shell, posix_spawn_direct, pipe2_cloexec, popen\n");
    return 1;
  }
