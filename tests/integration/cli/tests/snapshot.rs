...
fn test_snapshot_condvar() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/example-condvar.wasm"));
...
fn test_snapshot_default_file_system_tree() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .arg("ls")
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/coreutils.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_cowsay() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .stdin_str("blah\n")
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/cowsay.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_epoll() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/example-epoll.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_file_copy() {
    let snapshot = TestBuilder::new()
        ...
        .arg("/dev/stdin")
        .arg("/dev/stdout")
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/example-file-copy.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_execve() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .use_coreutils()
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/example-execve.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_readdir_tree() {
    let snapshot = TestBuilder::new()
        ...
        .use_coreutils()
        .args(["/"])
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/example-readdir_tree.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_minimodem_tx() {
    let mut snapshot = TestBuilder::new()
        ...
        .arg("--float-samples")
        .arg("same")
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/minimodem.wasm"));
...
fn test_snapshot_minimodem_rx() {
    let snapshot = TestBuilder::new()
        ...
        .arg("--float-samples")
        .arg("same")
        .stdin(include_bytes!("../../../../wasmer-test-files/integration/wasm/minimodem.data"))
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/minimodem.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_tokio() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/example-tokio.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_longjump() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .use_coreutils()
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/example-longjmp.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_fork() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .use_coreutils()
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/example-fork.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_longjump_fork() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/example-fork-longjmp.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_multithreading() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/example-multi-threading.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_wasi_threads() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .enable_threads(true)
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/wasi-threads.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_threaded_memory() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/threaded-memory.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_sleep() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/example-sleep.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_process_spawn() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .use_coreutils()
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/example-spawn.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_thread_locals() {
    let mut snapshot = TestBuilder::new()
        .with_name(function!())
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/example-thread-local.wasm"));
...
fn test_snapshot_vfork() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .use_coreutils()
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/example-vfork.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_signals() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/example-signal.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_dash_echo() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .stdin_str("echo 2")
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/dash.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_python_3_11_3() {
    let snapshot = TestBuilder::new()
        ...
        .arg("-c")
        .arg("print(10)")
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/python-3.11.3.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_bash_echo() {
    let snapshot = TestBuilder::new()
        ...
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/bash.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_catsay() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .stdin_str("meoooww")
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/catsay.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_quickjs() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .stdin_str("print(2+2);\n\\q\n")
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/qjs.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_fs_rename() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/fs-rename.wasm"));
    assert_json_snapshot!(snapshot);
}
...
fn test_snapshot_exit_0_from_main() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/exit-0-from-main.wasm"));
    assert!(matches!(
        snapshot.result,
...
fn test_snapshot_exit_1_from_main() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/exit-1-from-main.wasm"));
    assert!(matches!(
        snapshot.result,
...
fn test_snapshot_exit_0_from_worker() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/exit-0-from-worker.wasm"));
    assert!(matches!(
        snapshot.result,
        TestResult::Success(TestOutput { exit_code: 0, .. })
    ));
...
fn test_snapshot_exit_1_from_worker() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/exit-1-from-worker.wasm"));
    assert!(matches!(
        snapshot.result,
        TestResult::Success(TestOutput { exit_code: 1, .. })
    ));
...
fn test_snapshot_worker_terminating_normally() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/worker-terminating-normally.wasm"));
    assert!(matches!(
        snapshot.result,
        TestResult::Success(TestOutput { exit_code: 0, .. })
    ));
...
fn test_snapshot_worker_panicking() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/worker-panicking.wasm"));
    assert!(matches!(
        snapshot.result,
...
fn test_snapshot_mkdir_rename() {
    let snapshot = TestBuilder::new()
        .with_name(function!())
        .run_wasm(include_bytes!("../../../../wasmer-test-files/integration/wasm/mkdir-rename.wasm"));
    assert_json_snapshot!(snapshot);
}
