...
fn wasmer_init_works_2() -> anyhow::Result<()> {
    ...
    if cargo_wapm_present {
        ...
        pretty_assertions::assert_eq!(std::fs::read_to_string(path.join("Cargo.toml")).unwrap(), include_str!("./fixtures/init2.toml"));
        let read = std::fs::read_to_string(path.join("wapm.toml")).unwrap().lines().collect::<Vec<_>>().join("\n");
        let target = include_str!("./fixtures/init4.toml").lines().collect::<Vec<_>>().join("\n");
        pretty_assertions::assert_eq!(read.trim(), target.trim());
    }
