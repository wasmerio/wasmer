...
pub fn compile(file: &str, ignores: &HashSet<String>) -> Option<String> {
    ...
    let rust_test_filepath = format!(
        concat!(env!("CARGO_MANIFEST_DIR"), "/../emtests/{}.rs"),
    ...
    let module_path = format!("../emscripten_resources/emtests/{}.wasm", module_name);
    let test_output_path = format!(
        "../emscripten_resources/emtests/{}.{}",
        module_name, output_extension
    );
    if !Path::new(&format!("{}/{}", env!("CARGO_MANIFEST_DIR"), &module_path)).is_file() {
    ...
    .is_file()
    {
        println!("Path not found to test output: {}", test_output_path);
        None
    } else {
        let contents = format!(
            "#[test]{ignore}
...
pub fn build() {
    let rust_test_modpath = concat!(env!("CARGO_MANIFEST_DIR"), "/../emtests/mod.rs");
...
const IGNORE_LIST_PATH: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../emscripten_resources/emtests/ignores.txt"
);
