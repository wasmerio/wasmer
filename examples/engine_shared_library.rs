...
//!
//! This example illustrates how to use the `wasmer_engine_shared_library`,
//! aka the shared library engine. An engine applies roughly 2 steps:
//!
...
//!
//! In the particular context of the shared library engine, the
//! executable code is stored in a native shared object library.
//!
...
"#
    ...
    //
    // In this case, the engine is `wasmer_engine_shared_library` which means
    // that a shared object library is going to be generated.
    ...
    // stored into a shared object library by the engine.
...
fn test_engine_shared_library() -> Result<(), Box<dyn std::error::Error>> {
    main()
}
